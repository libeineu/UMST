...
class BPEWORDFusionMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights_bpe = torch.bmm(q_bpe, k_bpe.transpose(1, 2))
        attn_weights_bpe = self.apply_sparse_mask(attn_weights_bpe, tgt_len_bpe, src_len_bpe, bsz)

        attn_weights_word = torch.bmm(q_word, k_word.transpose(1, 2))
