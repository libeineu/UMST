...
class RelativeMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            if not self.tpu:
                relative_attn_weights = relative_attn_weights.masked_fill(
                    key_padding_mask.unsqueeze(1).unsqueeze(2).to(torch.bool),
                    float("-inf"),
                )
            else:
                relative_attn_weights = relative_attn_weights.transpose(0, 2)
                relative_attn_weights = relative_attn_weights.masked_fill(key_padding_mask, float("-inf"))
                relative_attn_weights = relative_attn_weights.transpose(0, 2)
            relative_attn_weights = relative_attn_weights.view(bsz * self.num_heads, tgt_len, src_len)

        if before_softmax:
            return relative_attn_weights, v

        attn_weights_float = utils.softmax(
            relative_attn_weights, dim=-1, onnx_trace=self.onnx_trace
        )
        relative_attn_weights = attn_weights_float.type_as(relative_attn_weights)
        attn_probs = self.dropout_module(relative_attn_weights)
